						...
						unsigned long *target_freq, u32 flags)
{
	...
	limited_idx = gpufreq_get_oppidx_by_freq(
			TARGET_DEFAULT,
...
{
	int opp_num = 0;
	const struct gpufreq_opp_info *opp_info = NULL;
	struct kbase_device *kbdev = (struct kbase_device *)mtk_common_get_kbdev();
	int i;
	unsigned long tmp_volt = 0;
	unsigned long tmp_freq = 0;

	opp_info = gpufreq_get_working_table(TARGET_DEFAULT);
	opp_num = gpufreq_get_opp_num(TARGET_DEFAULT);

	if (!opp_info) {
		dev_info(kbdev->dev, "Failed to get opp info\n");
		return;
	}

	if (opp_num <= 0) {
		dev_info(kbdev->dev, "Failed to get opp num\n");
		return;
	}

	for (i = 0; i < opp_num; i++)
	{
		tmp_volt = (unsigned long)opp_info[i].volt * 10;
		tmp_freq = (unsigned long)opp_info[i].freq * 1000;
		if (tmp_volt == 0 || tmp_freq == 0)
			continue;
		dev_pm_opp_adjust_voltage(kbdev->dev, tmp_freq, tmp_volt, tmp_volt, tmp_volt);
	}
}
