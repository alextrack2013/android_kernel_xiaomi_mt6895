	...
	struct adapter_power_cap *tacap)
{
	...
	int nopps_apdo_max = 0;
	int pps_apdo_max = 0;
	int nopps_ibus_max = 0;
	int pps_ibus_max = 0;
	int ibus_max = 0;
#endif
	struct mtk_pd_adapter_info *info;
...
APDO_REGAIN:
		...
		if (pd_cap.nr != 0) {
			...
			for (i = 0; i < pd_cap.nr; i++) {
				...
				#if defined (CONFIG_TARGET_PRODUCT_ARISTOTLE)
					if (tacap->type[i] == MTK_PD_APDO) {
						/*some 3thd pps only 16v or 22v max_voltage, fix 67w can't request this pdos*/
						if(tacap->max_mv[i] > 11000) {
							tacap->max_mv[i] = 11000;
							tacap->maxwatt[i] = tacap->max_mv[i]*tacap->ma[i];
						}
						if (tacap->maxwatt[i] > pps_apdo_max) {
							pps_apdo_max = tacap->maxwatt[i];
							pps_ibus_max = tacap->ma[i];
						}
					} else {
						if (tacap->maxwatt[i] > nopps_apdo_max) {
							nopps_apdo_max = tacap->maxwatt[i];
							nopps_ibus_max = tacap->ma[i];
						}
					}
					apdo_max = pps_apdo_max ? pps_apdo_max : nopps_apdo_max;
					ibus_max = pps_apdo_max ? pps_ibus_max : nopps_ibus_max;
				#else
			...
			#if defined (CONFIG_TARGET_PRODUCT_ARISTOTLE)
				apdo_max = ssdev_typec_filter_apdo_power_for_report(apdo_max, ibus_max);
			#else
